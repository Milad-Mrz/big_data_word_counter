...
public class P1A3_HadoopWordStripes extends Configured implements Tool {
    public static class Map extends Mapper<LongWritable, Text, Text, MapWritable> {
        ...
        public static void stripe(String word, MapWritable map) {
            LongWritable count = new LongWritable(0);
            if (map.containsKey(new Text(word))) {
                count = (LongWritable) map.get(new Text(word));
                map.remove(new Text(word));
            }
            count = new LongWritable(count.get() + one.get());
            map.put(new Text(word), count);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJarByClass(P1A3_HadoopWordStripes.class);
    ...
    public static void main(String[] args) throws Exception {
        int ret = ToolRunner.run(new Configuration(), new P1A3_HadoopWordStripes(), args);
        System.exit(ret);
    }
}
